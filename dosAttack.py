...
def TCPScan(dest="127.0.0.1", sourceport=65000,destport=80):
    ...
    src_port = sourceport
    dst_port=destport
    stealth_scan_resp = sr1(IP(dst=dst_ip)/TCP(sport=src_port,dport=dst_port,flags="S")/"dosAtackPayload",timeout=10)
    if(str(type(stealth_scan_resp))=="<class 'NoneType'>"):
        return "Filtered"
    elif(stealth_scan_resp.haslayer(TCP)):
        if(stealth_scan_resp.getlayer(TCP).flags == 0x12):
            send_rst = sr(IP(dst=dst_ip)/TCP(sport=src_port,dport=dst_port,flags="R")/"dosAtackPayload",timeout=10)
            return "Open"
...
if __name__ == "__main__":
    print(RangeScan(dest="10.0.2.15", startPort=5555,endPort=5555))